...
--A FUNCTION to TRUNCATE all data from all tables (SELECT truncate_schema('public');)
    ...
    $BODY$
        ...
        begin
            for selectrow in
                ...
                from 
                    (SELECT table_name FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_schema = _schema)t
            loop
